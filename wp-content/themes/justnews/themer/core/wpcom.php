...
class WPCOM {
    ...
    public static function modules_update( $res ){
        if( isset($res['changeset_status']) && $res['changeset_status']=='publish'
            && isset($res['setting_validities']) && isset($res['setting_validities']['_page_modules'])
            && $res['setting_validities']['_page_modules']
        ) {
            $customized = stripslashes($_POST['customized']);
            $customized = json_decode($customized, 1);
